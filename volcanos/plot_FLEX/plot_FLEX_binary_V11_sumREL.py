        ...
        for date in dfile:
            ...
            if os.path.isfile(FLEXPARTOutputDir + 'grid_' + outputFiletype + '_' + str(current_time)+'_001'):
                ...
                for sp in num_SPECIES:
                    ...
                    #print('#RELEASES: '+str(np.shape(np.array(grid))))
                    for rel in range(RELEASE):
                        theHorizslice+=grid[:,:,LEVEL,rel,0] # last dimension is AGECLASS -> hard coded for one AGECLASS
                        theHorizslice_wetdep+=wetdep[:,:,rel,0]
                        theHorizslice_drydep+=drydep[:,:,rel,0]
                ...
                if theHorizslice.max().max() >= plot_thres:
                    ...
                    cbar.set_label('Total Column Ash Concentration [g/m3]')
                ...
                filename = savepath + '2D_' + str(current_time) + '_species' + str(num_SPECIES_help)
                if isNested == False:
                    filename+='.png'
                else:
                    filename+='_nest.png'
